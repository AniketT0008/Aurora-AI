...
def analyze_behavior(data, productivity_data=None):
    ...
    # Cross-pollinate with productivity data for better burnout detection
    deep_work = safe_float(productivity_data.get("deep_work_hours", 0)) if productivity_data else 0
    
    sleep_deficit = max(0.0, 8.0 - sleep)
    # Severe sleep deprivation (under 4h) causes exponential risk
    if sleep < 4:
        sleep_deficit *= 1.5
    ...
    # Workload impact on burnout
    work_load_penalty = (deep_work / 6.0) * 20.0 # 6h+ deep work starts adding risk
    if deep_work > 10:
        work_load_penalty += 20.0 # Extreme work hours

    burnout_risk = (sleep_deficit / 4.0) * 40 + (max(0.0, 3.0 - exercise) / 3.0) * 15 + activity_gap * 10 + note_penalty + work_load_penalty - note_bonus
